...
- `download.py` — the downloader (fetches hourly ticks from the Dukascopy API with per-chunk progress logs, automatically falling back to the classic bi5 archive feed for hours the API rejects)
...
- `download.py` — دانلودر (تیک‌های ساعتی را با لاگ پیشرفت chunk‌به‌chunk از API دوکاسکوپی می‌گیرد و برای ساعت‌هایی که API رد می‌کند به‌صورت خودکار به فید آرشیو کلاسیک bi5 برمی‌گردد)
...
- `download.py` — загрузчик (скачивает почасовые тики из API Dukascopy с логом прогресса по чанкам и автоматическим откатом на классический архив bi5 для часов, которые API отклоняет)
...
- `download.py` — أداة التنزيل (تجلب التيكات ساعة بساعة من واجهة Dukascopy مع سجل تقدم لكل مقطع وتراجع تلقائي إلى أرشيف bi5 الكلاسيكي للساعات التي ترفضها الواجهة)
...
- `download.py` — el descargador (obtiene los ticks por hora de la API de Dukascopy con logs de progreso por bloques y respaldo automático en el archivo clásico bi5 para las horas que la API rechaza)
